...
# SIZE: the size of the files to transfer
...
cp ftpcheck${SIZE} /tmp/ftpcheck.$SHELLPID
#FILEPATH='ftpcheck.$SHELLPID'
}
...
if [ ${#} -ne ${EXP_ARGS} ]; then
  f_unk "Expected ${EXP_ARGS} arguments, but received ${#} -- Usage: $0 username password port host [24|64|1024]"
fi
